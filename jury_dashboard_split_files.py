...
# Load split Excel files
@st.cache_data
def load_data():
    crime_df = pd.read_excel("CrimeCounties.xlsx")
    demo_df = pd.read_excel("CountyDemographics.xlsx").rename(columns={"Unnamed: 0": "County"})
    pol_df = pd.read_excel("CountyPolitics.xlsx")
    city_df = pd.read_excel("CityCrimeFinal.xlsx")
    citypol_df = pd.read_excel("CityPolitics.xlsx")
...
if not demo_row.empty:
    ...
    col2.metric("Median Earnings (25+)", f"${int(demo_row.iloc[0, 2]):,}")
