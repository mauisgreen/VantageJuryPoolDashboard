...
@st.cache_data
def load_data():
    xls = pd.ExcelFile("CaliforniaCountyData2023Final.xlsx")
    crime_df = xls.parse("CrimeCounties")
    demo_df = xls.parse("CountyDemographics").rename(columns={"Unnamed: 0": "County"})
    pol_df = xls.parse("CountyPolitics")
    city_df = xls.parse("CityCrimeFinal")
    citypol_df = xls.parse("CityPolitics")
